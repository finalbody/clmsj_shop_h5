		...
		<view class="login_tips" v-if="!userInfo.id && (showLogin==1 && showLoginBox==1)">
			...
			<view class="mask-box" :class="{'full' : fullLogin==1}">
				...
				<view class="login-btn">
					<button type="warn" openType="getUserInfo" lang="zh_CN" @getuserinfo="getUserInfo" withCredentials="true"><text>立即登陆</text></button>
					<button v-if="onlyLogin==0" type="default" @tap="hideLoginBox()"><text>暂不登陆</text></button>
				</view>
			</view>
		</view>
	</view>
...
.login_tips{
	padding: 0px;
	.mask-bg{
		left: 0;right: 0;bottom: 50px; background: #303133;
		opacity: 0.5; z-index: 9998;
		position: fixed;
		padding-top: 30px;
		height: 240px;
	}
	.mask-bg-full{
		left: 0;right: 0;bottom: 0;top:0; background: #303133;
		opacity: 0.5; z-index: 9998;
		position: fixed;
	}
	.mask-box{
		position: fixed;bottom: 70px; left: 5%;right: 5%;z-index: 9999;background: #ffffff;padding: 20px;min-height: 150px;border-radius: 5px;
	}
	.full{
		bottom: calc(50% - 75px); 
	}
