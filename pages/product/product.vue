    ...
    <view class="container">
				...
				<scroll-view class="floor-list" scroll-x>
					<view class="scoll-wrapper">
						<block v-for="(item, index) in hotGoodsList" :key="index">
							<view v-if="index < 5 && item.goods_photo[0].img_url" class="floor-item" @tap="detailBtn(item.goods_id)">
					...
					<view class="scoll-wrapper">
						<block v-for="(item, index) in hotGoodsList" :key="index">
							<view v-if="index >= 4 && item.goods_photo[0].img_url" class="floor-item" @tap="detailBtn(item.goods_id)">
								<image :src="item.goods_photo[0].img_url" mode="aspectFill" />
							</view>
						</block>
					</view>
		...
		<view class="detail-desc">
			...
			<view>
				<view v-for="(item, index) in imgList" :key="index">
					<image mode="widthFix" style="width: 100%;" :src="item" :data-src="item" @tap="previewImage(imgList,item)"></image>
				</view>
			</view>
		</view>
		...
		<view class="contact-box">
			<button class="btn-share" open-type="share">
				<view class="yticon icon-share1"></view>
				分享</button>
		</view>
				...
				<view class="a-t">
					...
					<view class="right">
						...
						<text class="stock">
							...
							<text v-else>{{goods.stock_num}}</text>
							件
						</text>
						<view class="selected">
							已选：
							<text class="selected-text" v-if="specSelected.size">{{specSelected.size}}</text>
							<text class="selected-text" v-if="specSelected.color">{{specSelected.color}}</text>
						</view>
					</view>
				</view>
...
export default {
    ...
    async onLoad(options) {
        ...
        if(share_openid){
            const res = await app.request(this.$Url + "/clmsj/login/getLsbUid", {
                openid: share_openid
            });
        ...
        const res = await app.request(this.$Url + "/clmsj/goods/goodsdetail_shop", {
            goods_id : goods_id,
			shop_id : this.sid
        });
    ...
    methods: {
        ...
        selectSpec(subitem, index) {
			...
			this.goods.storageList.forEach(item=>{
				if(item.color == this.specSelected.color && item.size == this.specSelected.size){
