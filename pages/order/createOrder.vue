<template>
	<view>
		...
		<navigator url="/pages/address/address?source=1" class="address-section">
			<view class="order-content">
				...
				<view class="cen" v-if="addressData.name">
					...
					<text class="address">{{addressData.address}} {{addressData.home_number}}</text>
				</view>
				<view class="cen" v-else>
					<view class="top">
						<text class="name">点击设置收货地址信息</text>
		...
		<view class="goods-section" v-if="goods.goods_id">
			<!-- 商品列表 -->
			<view class="g-item">
				<image :src="goods.goodsphoto_image[0]"></image>
				<view class="right">
		...
		<view class="yt-list">
			<view class="yt-list-cell b-b">
				...
				<text class="cell-tip"><text class="price">￥{{goods.shop_total_price}}</text></text>
			</view>
		...
		<uni-login  ref="uniLogin"  type="login" v-on:pLoadData="loadData" :showLogin="showLogin" :onlyLogin='1'></uni-login>
	</view>
</template>
		...
		methods: {
			...
			submit(){
				if(this.order_id == 0){
					...
					uni.request({
						url: this.$Url + '/clmsj/order/saveOrder',
						data: {
							shop_id : this.sid,
